# VMS — AI Harness & Workflow Guide (Codex CLI)

This document is loaded automatically by Codex CLI from `~/.codex/AGENTS.md`.
...
Skills are installed at: `~/.codex/skills/<skill-name>/SKILL.md`

Codex CLI loads skills from `~/.codex/skills/` automatically each session.
To invoke a skill, reference it by name — e.g., *"Use skill-plan-mode-execution"*.

## Codex CLI specifics

- Global instruction file: `~/.codex/AGENTS.md` — Codex reads this as the system prompt on every session.
- Place `AGENTS.md` in the **service repo root** as well — loaded every session in addition to the global file. Append the VMS block, do not overwrite existing content.
...
- MCP config: `~/.codex/config.json` — field `mcpServers`
- Codex CLI runs in a sandboxed environment with auto-approval, network, or manual approval modes. Use `--approval-mode auto-edit` for agentic tasks. Sandboxing varies by OS (macOS: full sandbox; Linux: no sandbox by default).
...
Codex is one of the v1 projection targets for the shared orchestration source at
