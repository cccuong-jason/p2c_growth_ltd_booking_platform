# VMS — AI Harness & Workflow Guide (Claude Code)

This document is loaded automatically by Claude Code from `~/.claude/CLAUDE.md`.
...
Claude Code loads skills from `~/.claude/skills/` automatically each session.
To invoke a skill, reference it by name — e.g., *"Use skill-plan-mode-execution"*.

## Claude Code specifics

- Place `CLAUDE.md` in the **service repo root** as well — loaded automatically every session.
...
Claude is one of the v1 projection targets for the shared orchestration source at
