...
export default function AutomationDashboard() {
  const records = [
    { id: "AUT-501", business: "Royal Nails & Spa", stack: "Email + Booking", complexity: "Standard", status: "Active", date: "2026-06-01" },
    { id: "AUT-502", business: "The Steakhouse", stack: "Workflow Automation", complexity: "Advanced", status: "In Testing", date: "2026-05-28" },
  ];
                ...
                <tr key={item.id} className="transition-colors hover:bg-slate-50/50 group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="font-bold text-slate-900">{item.business}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 font-medium text-slate-600">{item.stack}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest ${
                      item.complexity === 'Advanced' ? 'bg-purple-50 text-purple-600 border border-purple-100' : 'bg-blue-50 text-blue-600 border border-blue-100'
                    } border`}>
                      {item.complexity}
                    </span>
                  </td>
